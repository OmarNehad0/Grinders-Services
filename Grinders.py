    ...
    try:
        ...
        for entry in skill_entries:
            ...
            try:
                ...
                await ctx.send(f"Invalid format for entry '{entry}'. Use skill start-end like 'agility 1-99'.")
                continue
            ...
            for i, chunk in enumerate(breakdown_chunks):
                embed.add_field(name="**This will consist of the following methods:**" if i == 0 else "\u200b", value=chunk, inline=False)

            # --- Chunked Alternative Method Costs ---
            alt_text = "\n".join([
            ...
            for i, chunk in enumerate(alt_chunks):
                embed.add_field(name="**__Alternatively, if you want to choose a specific method__**" if i == 0 else "\u200b", value=chunk, inline=False)
        ...
        summary_embed.add_field(name="Total GP", value=f"{total_gp_sum:,.2f}M")
        summary_embed.add_field(name="Total USD", value=f"${total_usd_sum:,.2f}")
    ...
    for file_name, emoji in json_files.items():
        ...
        for item in data:
            ...
            if not emoji_value:
                ...
                # Case 1: numeric ID (custom emoji by ID only)
                if emoji_value.isdigit():
                    emoji_value = f"<:icon:{emoji_value}>"

                # Case 2: full custom emoji format <:name:ID>
                elif emoji_value.startswith("<:") and emoji_value.endswith(">"):
                    # already valid
                    pass

                # Case 3: standard Unicode emoji (🔥, 💎, ⚔️, etc.)
                elif any(ord(ch) > 10000 for ch in emoji_value):
                    # keep as is (emoji symbols)
                    pass

                # Otherwise, treat it as invalid or plain text
                else:
                    emoji_value = None
